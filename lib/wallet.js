...
class Wallet {
  constructor(options) {
    ...
    this.isLocked = !seed;
    this.denomination = options.token ? options.token.symbol : 'BNB';
    this.baseDenomination = 'BNB';
    this.name = options.name || 'Binance Smart Chain';
  ...
  async load() {
    ...
    if (this.token) {
      this.bscBalance = helpers.min(results[3].confirmedBalance, results[3].balance);
    }
  }
  async update() {
