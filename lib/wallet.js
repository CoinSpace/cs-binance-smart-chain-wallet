...
class Wallet {
  ...
  constructor(options) {
    ...
    this.settings.bip44 = this.settings.bip44 || "m/44'/714'/0'";

    let apiHistory;
    if (options.useTestNetwork) {
      this.chainId = 97; // 0x61
      this.networkId = 97;
      this.#txUrl = 'https://testnet.bscscan.com/tx/${txId}';
      apiHistory = 'https://api-testnet.bscscan.com/';
    } else {
      this.chainId = 56; // 0x38
      this.networkId = 1;
      this.#txUrl = 'https://bscscan.com/tx/${txId}';
  ...
  async load() {
    ...
    if (this.crypto.type === 'token') {
      promises = [
        ...
        this.api.common.getBscscanApiKey(),
